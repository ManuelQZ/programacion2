...
public class App {
    ...
    public static void calcularDeudaCliente(Cliente cliente) {
      ArrayList<Reserva> reservas = cliente.getReserva();
        Deuda = 0;
        for (int j=0 < reservas.size(); j++){
          Reserva reserva = reservas.get(j);
          ArrayList<Servicio> lista_servicios = reserva.getHabitacion();
          Habitacion habitacion = reserva.getHabitacion();
          Deuda = Deuda + precio;
          System.out.println(i+1+)"." + lista_servicios.get(i) + "." + Deuda);
        }
    }
